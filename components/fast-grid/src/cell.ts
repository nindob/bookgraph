...
export class StringCell implements CellComponent {
  ...
  constructor(id: number, offset: number, text: string | number) {
    ...
    // NOTE: fonts are super expensive, might be more simple fonts that are faster to render? testing to render a cursive text with subpixel antialiasing, vs
    ...
    // NOTE: align-items actually has a super slight imapact on Layerize time, using padding for now
    this.el.className =
